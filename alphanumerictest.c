...
void labels1()
{
   ...
   const char *people[nx] = {"Jean","Pierre","Marie","Odile",
      ...
      "Philippe","Suzanne","Jeff","Valery"}; /// list of lables for the histogram


   ///creates new canvas (not to be used)
   TCanvas *c1 = new TCanvas("c1","demo bin labels",10,10,900,500);
   c1->SetGrid();
   c1->SetBottomMargin(0.15);

   ///generates a new histogram 
   TH1F *h = new TH1F("h","test",nx,0,nx); ///h is the pointer to the histogram 
   ...
   for (i=1;i<=nx;i++) h->GetXaxis()->SetBinLabel(i,people[i-1]); /// important
